...
export default function Experience() {
    ...
    const ctx = gsap.context(() => {
      if (pathRef.current) {
        ...
        gsap.to(pathRef.current, {
          strokeDashoffset: 0,
          ease: "none",
          scrollTrigger: { trigger: wrapRef.current, start: "top 75%", end: "bottom 60%", scrub: 0.5 },
        });
      }
      ...
      cardRefs.current.forEach((el, i) => {
        ...
        if (stamp) {
          gsap.set(stamp, { autoAlpha: 0, scale: 0.6, rotate: -20 });
          gsap.to(stamp, {
            autoAlpha: 1,
            scale: 1,
            rotate: -3,
  ...
  return (
    <section id="experience" className="relative overflow-hidden bg-cream px-6 py-24 text-navy">
      ...
      <div ref={wrapRef} className="relative mx-auto mt-16 max-w-4xl pl-8 sm:pl-12">
        <svg
          aria-hidden="true"
          viewBox="0 0 4 100"
        ...
        >
          <path
            ref={pathRef}
            d="M2 0 C 0 25, 4 50, 2 75 C 0 90, 4 95, 2 100"
            stroke="#173DE5"
            strokeWidth="3"
            fill="none"
          />
            ...
            <div ref={(el) => (cardRefs.current[i] = el)}>
              {stage.promotion && (
                <span
                  ...
                  className="mb-4 inline-block -rotate-3 rounded-sm bg-signal px-3 py-1 font-note text-lg text-navy shadow-[0_6px_16px_rgba(12,12,12,0.2)]"
                >
                  Promoted ↑
                </span>
              )}
